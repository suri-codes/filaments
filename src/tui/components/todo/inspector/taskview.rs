...
impl From<(&Task, &Index)> for TaskView<'_> {
    fn from(value: (&Task, &Index)) -> Self {
        let task = value.0;
        let idx = value.1;

        let mut name = TextArea::new(vec![task.name.clone()]);
        name.set_block(Block::bordered().title("[N]ame"));
        name.set_cursor_style(Style::reset());
        name.set_cursor_line_style(Style::reset());

        let mut priority = TextArea::new(vec![task.priority.to_string()]);
        ...
        let due_finished_at = {
            let (title, content) = task.finished_at().map_or_else(
                || ("[D]ue", task.due.to_string()),
        ...
        let preview = idx.get_zod(&task.zettel.id).body.clone().into();

        Self {
            name,
            priority,
            due_finished_at,
            parent_group: Paragraph::new(task.group.name.clone())
